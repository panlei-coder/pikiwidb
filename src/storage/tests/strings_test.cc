...
TEST_F(StringsTest, BitOpTest) {
  ...
  // AND
  s = db.BitOp(storage::BitOpType::kBitOpAnd, "BITOP_DESTKEY", src_keys, std::ref(value_to_dest), &ret);
  ASSERT_TRUE(s.ok());
  ASSERT_EQ(ret, 7);
  ...
  // OR
  s = db.BitOp(storage::BitOpType::kBitOpOr, "BITOP_DESTKEY", src_keys, std::ref(value_to_dest), &ret);
  ASSERT_TRUE(s.ok());
  ASSERT_EQ(ret, 7);
  s = db.Get("BITOP_DESTKEY", &value);
  ...
  // XOR
  s = db.BitOp(storage::BitOpType::kBitOpXor, "BITOP_DESTKEY", src_keys, std::ref(value_to_dest), &ret);
  ASSERT_TRUE(s.ok());
  ASSERT_EQ(ret, 7);
  s = db.Get("BITOP_DESTKEY", &value);
  ...
  s = db.BitOp(storage::BitOpType::kBitOpNot, "BITOP_DESTKEY", not_keys, std::ref(value_to_dest), &ret);
  ASSERT_TRUE(s.ok());
  ASSERT_EQ(ret, 6);
  s = db.Get("BITOP_DESTKEY", &value);
  ASSERT_STREQ(value.c_str(), "\xb9\xb0\xb0\xbd\xbe\xad");
  // NOT operation more than two parameters
  s = db.BitOp(storage::BitOpType::kBitOpNot, "BITOP_DESTKEY", src_keys, std::ref(value_to_dest), &ret);
