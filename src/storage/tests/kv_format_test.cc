...
TEST(KVFormatTest, BaseKeyFormat) {
  ...
  ASSERT_EQ(slice_enc, rocksdb::Slice(expect_enc));
...
TEST(KVFormatTest, BaseDataKeyFormat) {
  ...
  ASSERT_EQ(seek_key_enc, rocksdb::Slice(expect_enc));

  rocksdb::Slice key_enc = bdk.Encode();
  expect_enc.append(16, '\0');
  ASSERT_EQ(key_enc, rocksdb::Slice(expect_enc));
...
TEST(KVFormatTest, ListDataKeyFormat) {
  ...
  storage::EncodeFixed64(dst, version);
  expect_enc.append(dst, 8);
  storage::EncodeFixed64(dst, index);
  expect_enc.append(dst, 8);
  expect_enc.append(16, '\0');
  ASSERT_EQ(key_enc, rocksdb::Slice(expect_enc));
